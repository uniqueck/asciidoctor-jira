...
function jiraIssuesBlockMacro (context) {
  return function () {
    ...
    self.process((parent, target, attrs) => {
      ...
      doc.getLogger().info(`Searching issues with jql: ${jql} and custom fields: ${customFields}.`)
      ...
      for (let i = 0; i < issues.length; i++) {
        ...
        const idColumn = `a|${createLinkToIssue(jiraConfig.frontendUrl, issue.key)}[${issue.key}]`

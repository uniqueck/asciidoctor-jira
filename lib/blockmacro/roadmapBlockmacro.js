...
function roadmapBlockMacro (name, context) {
  return function () {
    ...
    self.process((parent, target, attrs) => {
      ...
      const config = require('./roadmapBlockmacroConfig').createConfig(name, parent, target, attrs)
      ...
      const jiraClient = new JIRA(doc, config.jira)
      ...
      for (const catIndex in categories) {
        ...
        if (issues != null && issues.length > 0) {
          ...
          for (const issueIndex in issues) {
            ...
            const jiraIssueLink = `${config.jira.frontendUrl}/browse/${issueKey}`
      ...
      if (totalIssues === 0) {
        doc.getLogger().warn(`No issues found for roadmap ${jiraProject} ${year}, skipping PlantUML diagram generation.`)
        return undefined
      }

      doc.getLogger().debug(`Content:\n${content.join('\n')}`)

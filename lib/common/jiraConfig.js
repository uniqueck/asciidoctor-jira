...
//
// Generated by LF-ET 2.3.0 (231102a), https://www.lohrfink.de/lfet
// From decision table
  ...
  do {
    ...
    if (_step === 1) {
      ...
      if (macroName !== '') {
        ...
        // Trace ---->
        logger.info(`jiraConfig - 20240128.104307 - 1 / 36 - ${JSON.stringify(_jiraConfig)}`)
        // Trace <----
        ...
        _jiraconfigRuleExecuted = true
        // Trace ---->
        logger.info(`jiraConfig - 20240128.104307 - 2 / 36 - ${JSON.stringify(_jiraConfig)}`)
        // Trace <----
        // Action A05/01: next step / + / next step
        _step++
        _exit = false
    ...
    // Condition B01/02: Step / 2 / jira base url information
      ...
      if (projectKey) {
        ...
        if (attrs.jiraBaseUrl) {
          ...
          _jiraconfigRuleExecuted = true
          // Trace ---->
          logger.info(`jiraConfig - 20240128.104307 - 3 / 36 - ${JSON.stringify(_jiraConfig)}`)
          // Trace <----
        ...
        // Condition B04/02: base url with project key prefix is defined / DOC / via doc attr
          ...
          _jiraconfigRuleExecuted = true
          // Trace ---->
          logger.info(`jiraConfig - 20240128.104307 - 4 / 36 - ${JSON.stringify(_jiraConfig)}`)
          // Trace <----
        ...
        // Condition B04/03: base url with project key prefix is defined / ENV / via env
          ...
          _jiraconfigRuleExecuted = true
          // Trace ---->
          logger.info(`jiraConfig - 20240128.104307 - 5 / 36 - ${JSON.stringify(_jiraConfig)}`)
          // Trace <----
          ...
          if (attrs.jiraBaseUrl) {
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 6 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
            // Action A02/01: set base url / ATTRS / via macro attrs
            _jiraConfig.baseUrl = attrs['jira-baseurl']
            // Action A05/01: next step / + / next step
          ...
          // Condition B05/02: base url without project key prefix is defined / DOC / via doc attr
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 7 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
          ...
          // Condition B05/03: base url without project key prefix is defined / ENV / via env
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 8 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 9 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
        ...
        if (attrs.jiraBaseUrl) {
          ...
          _jiraconfigRuleExecuted = true
          // Trace ---->
          logger.info(`jiraConfig - 20240128.104307 - 10 / 36 - ${JSON.stringify(_jiraConfig)}`)
          // Trace <----
          // Action A02/01: set base url / ATTRS / via macro attrs
          _jiraConfig.baseUrl = attrs['jira-baseurl']
          // Action A05/01: next step / + / next step
        ...
        // Condition B05/02: base url without project key prefix is defined / DOC / via doc attr
          ...
          _jiraconfigRuleExecuted = true
          // Trace ---->
          logger.info(`jiraConfig - 20240128.104307 - 11 / 36 - ${JSON.stringify(_jiraConfig)}`)
          // Trace <----
          // Action A02/02: set base url / DOC / via doc attr
          _jiraConfig.baseUrl = doc.getAttribute(`${_prefixDoc}jira-baseurl`)
          // Action A05/01: next step / + / next step
        ...
        // Condition B05/03: base url without project key prefix is defined / ENV / via env
          ...
          _jiraconfigRuleExecuted = true
          // Trace ---->
          logger.info(`jiraConfig - 20240128.104307 - 12 / 36 - ${JSON.stringify(_jiraConfig)}`)
          // Trace <----
          // Action A02/04: set base url / ENV / via env
          _jiraConfig.baseUrl = process.env[`${_prefixEnv}JIRA_BASEURL`]
          // Action A05/01: next step / + / next step
          ...
          _jiraconfigRuleExecuted = true
          // Trace ---->
          logger.info(`jiraConfig - 20240128.104307 - 13 / 36 - ${JSON.stringify(_jiraConfig)}`)
          // Trace <----
          // Action A04/01: log mandatory information is missing / URL / Jira base url is missing
          logger.error('Jira base url is missing')
          // Action A05/02: next step / E / Exit
      ...
      if (projectKey) {
        ...
        if (doc.getAttribute(`${projectKey.toLowerCase()}-${_prefixDoc}jira-username`)) {
          ...
          if (process.env[`${projectKey.toUpperCase()}_${_prefixEnv}JIRA_PASSWORD`]) {
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 14 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
            ...
            if (process.env[`${projectKey.toUpperCase()}_${_prefixEnv}JIRA_APITOKEN`]) {
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 15 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 16 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
        ...
        // Condition B06/02: username with project key prefix is defined / ENV / via env
          ...
          if (process.env[`${projectKey.toUpperCase()}_${_prefixEnv}JIRA_PASSWORD`]) {
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 17 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
            ...
            if (process.env[`${projectKey.toUpperCase()}_${_prefixEnv}JIRA_APITOKEN`]) {
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 18 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 19 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              // Action A04/03: log mandatory information is missing / PWD/API / Jira password or api token is missing
              logger.error('Jira password or api token is missing')
              // Action A05/02: next step / E / Exit
          ...
          if (doc.getAttribute(`${_prefixDoc}jira-username`)) {
            ...
            if (process.env[`${_prefixEnv}JIRA_PASSWORD`]) {
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 20 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              // Action A03/03: set auth / DOC_USR_PWD / username via doc attr and password via env without project key prefix
              _jiraConfig.auth = 'Basic ' + Buffer.from(doc.getAttribute(`${_prefixDoc}jira-username`) + ':' + process.env[`_${_prefixEnv}JIRA_PASSWORD`]).toString('base64')
              // Action A05/02: next step / E / Exit
              _exit = true
              // Rule R20 <----
            } else {
              ...
              if (process.env[`${_prefixEnv}JIRA_APITOKEN`]) {
                ...
                _jiraconfigRuleExecuted = true
                // Trace ---->
                logger.info(`jiraConfig - 20240128.104307 - 21 / 36 - ${JSON.stringify(_jiraConfig)}`)
                // Trace <----
                // Action A03/04: set auth / DOC_USR_API / username via doc attr and api token via env without project key prefix
                _jiraConfig.auth = 'Basic ' + Buffer.from(doc.getAttribute(`${_prefixDoc}jira-username`) + ':' + process.env[`_${_prefixEnv}JIRA_APITOKEN`]).toString('base64')
                // Action A05/02: next step / E / Exit
                _exit = true
                // Rule R21 <----
              } else {
                ...
                _jiraconfigRuleExecuted = true
                // Trace ---->
                logger.info(`jiraConfig - 20240128.104307 - 22 / 36 - ${JSON.stringify(_jiraConfig)}`)
                // Trace <----
                // Action A04/03: log mandatory information is missing / PWD/API / Jira password or api token is missing
                logger.error('Jira password or api token is missing')
                // Action A05/02: next step / E / Exit
          ...
          // Condition B07/02: username without project key prefix is defined / ENV / via env
            ...
            if (process.env[`${_prefixEnv}JIRA_PASSWORD`]) {
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 23 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              ...
              if (process.env[`${_prefixEnv}JIRA_APITOKEN`]) {
                ...
                _jiraconfigRuleExecuted = true
                // Trace ---->
                logger.info(`jiraConfig - 20240128.104307 - 24 / 36 - ${JSON.stringify(_jiraConfig)}`)
                // Trace <----
                ...
                _jiraconfigRuleExecuted = true
                // Trace ---->
                logger.info(`jiraConfig - 20240128.104307 - 25 / 36 - ${JSON.stringify(_jiraConfig)}`)
                // Trace <----
                // Action A04/03: log mandatory information is missing / PWD/API / Jira password or api token is missing
                logger.error('Jira password or api token is missing')
                // Action A05/02: next step / E / Exit
            ...
            if (process.env[`${projectKey.toUpperCase()}_${_prefixEnv}JIRA_PAT`]) {
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 26 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              ...
              if (process.env[`${_prefixEnv}JIRA_PAT`]) {
                ...
                _jiraconfigRuleExecuted = true
                // Trace ---->
                logger.info(`jiraConfig - 20240128.104307 - 27 / 36 - ${JSON.stringify(_jiraConfig)}`)
                // Trace <----
                ...
                _jiraconfigRuleExecuted = true
                // Trace ---->
                logger.info(`jiraConfig - 20240128.104307 - 28 / 36 - ${JSON.stringify(_jiraConfig)}`)
                // Trace <----
        ...
        if (doc.getAttribute(`${_prefixDoc}jira-username`)) {
          ...
          if (process.env[`${_prefixEnv}JIRA_PASSWORD`]) {
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 29 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
            // Action A03/03: set auth / DOC_USR_PWD / username via doc attr and password via env without project key prefix
            _jiraConfig.auth = 'Basic ' + Buffer.from(doc.getAttribute(`${_prefixDoc}jira-username`) + ':' + process.env[`_${_prefixEnv}JIRA_PASSWORD`]).toString('base64')
            // Action A05/02: next step / E / Exit
            ...
            if (process.env[`${_prefixEnv}JIRA_APITOKEN`]) {
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 30 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              // Action A03/04: set auth / DOC_USR_API / username via doc attr and api token via env without project key prefix
              _jiraConfig.auth = 'Basic ' + Buffer.from(doc.getAttribute(`${_prefixDoc}jira-username`) + ':' + process.env[`_${_prefixEnv}JIRA_APITOKEN`]).toString('base64')
              // Action A05/02: next step / E / Exit
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 31 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              // Action A04/03: log mandatory information is missing / PWD/API / Jira password or api token is missing
              logger.error('Jira password or api token is missing')
              // Action A05/02: next step / E / Exit
        ...
        // Condition B07/02: username without project key prefix is defined / ENV / via env
          ...
          if (process.env[`${_prefixEnv}JIRA_PASSWORD`]) {
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 32 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
            // Action A03/07: set auth / ENV_USR_PWD / username and password via env without project key prefix
            _jiraConfig.auth = 'Basic ' + Buffer.from(process.env[`${_prefixEnv}JIRA_USERNAME`] + ':' + process.env[`${_prefixEnv}JIRA_PASSWORD`]).toString('base64')
            // Action A05/02: next step / E / Exit
            ...
            if (process.env[`${_prefixEnv}JIRA_APITOKEN`]) {
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 33 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              // Action A03/08: set auth / ENV_USR_API / username and api token via env without project key prefix
              _jiraConfig.auth = 'Basic ' + Buffer.from(process.env[`${_prefixEnv}JIRA_USERNAME`] + ':' + process.env[`${_prefixEnv}JIRA_APITOKEN`]).toString('base64')
              // Action A05/02: next step / E / Exit
              ...
              _jiraconfigRuleExecuted = true
              // Trace ---->
              logger.info(`jiraConfig - 20240128.104307 - 34 / 36 - ${JSON.stringify(_jiraConfig)}`)
              // Trace <----
              // Action A04/03: log mandatory information is missing / PWD/API / Jira password or api token is missing
              logger.error('Jira password or api token is missing')
              // Action A05/02: next step / E / Exit
          ...
          if (process.env[`${_prefixEnv}JIRA_PAT`]) {
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 35 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
            // Action A03/10: set auth / ENV_PAT / personal access token via env without project key prefix
            _jiraConfig.auth = 'Bearer ' + process.env[`${_prefixEnv}JIRA_PAT`]
            // Action A05/02: next step / E / Exit
            ...
            _jiraconfigRuleExecuted = true
            // Trace ---->
            logger.info(`jiraConfig - 20240128.104307 - 36 / 36 - ${JSON.stringify(_jiraConfig)}`)
            // Trace <----
            // Action A04/02: log mandatory information is missing / USR/PAT / Jira username or personal access token is missing
            logger.error('Jira username or personal access token is missing')
            // Action A05/02: next step / E / Exit
    ...
    if (!_jiraconfigRuleExecuted) {
      // Trace ---->
      logger.info(`jiraConfig - 20240128.104307 - 0 / 36 - ${JSON.stringify(_jiraConfig)}`)
      // Trace <----
      // Error ---->
      // profile LFET.JavaScript.Error.ini not found
      // used LF-ET 2.3.0 (231102a) build in default
